...
public class Controller implements Observer {
	...
	public Controller() {
		...
		parser.getFactory().addObserver(this);
		curEnv = parser.getGlobalEnvironment(); // Only global environment at
												// the moment
	}
	...
	public List<Agent> getAllTurtles() { // The proper method that should be
											// called, allows for multiple
											// turtles
	...
	public void update(Observable o, Object arg) {
		int index = arg.toString().lastIndexOf('.');
		String myArg = arg.toString().substring(index+1);
		myArg = myArg.substring(0, 1).toLowerCase() + myArg.substring(1);
		try {
			gui.getDisplay().getClass().getDeclaredMethod(myArg, new Class<?>[0]).invoke(gui.getDisplay());
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		switch (myArg) {
//			case "SetScale":
//				gui.getDisplay().enableScaling();
//				break;
//			case "ClearScale":
//				gui.getDisplay().resetGridScale();
//				break;
//			default:
//				break;
//		}

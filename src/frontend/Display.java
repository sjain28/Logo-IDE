...
public class Display extends Window {
	...
	private void drawTurtle() {
		...
		for (Agent thisTurtle : myTurtles) {
			...
			if (outOfBounds(thisImageView)) {
				wasOutOfBounds = true;
				super.getRoot().getChildren().remove(thisImageView);
			} else {
				if (!super.getRoot().getChildren().contains(thisImageView)) {
					super.getRoot().getChildren().add(thisImageView);
				}
			}
	...
	private void increaseGridScale() {
		multFactor = multFactor * 2;
		adjustGridScale(multFactor);
	}
	...
	private void adjustGridScale(double factor) {
		...
		for (ImageView thisImageView : agentToPicture.values()) {
			...
			if (factor>1) {
				width = thisImageView.getFitWidth() / factor;
				height = thisImageView.getFitHeight() / factor;
			} else {
				width = TURTLE_WIDTH;
				height = TURTLE_HEIGHT;
			}
			
			thisImageView.setFitWidth(width);
			thisImageView.setFitHeight(height);
		}
	...
	private double imageViewOffsetX(double inVal, ImageView thisImageView) {
		return (inVal / multFactor + ORIGIN.getX() + thisImageView.getFitWidth()
				- thisImageView.getFitWidth() / multFactor);
	}

	private double lineOffsetX(double inVal, ImageView thisImageView) {
		return (inVal / multFactor + ORIGIN.getX() + thisImageView.getFitWidth() / 2 + thisImageView.getFitWidth()
				- thisImageView.getFitWidth() / multFactor);
	}

	private double imageViewOffsetY(double inVal, ImageView thisImageView) {
		return (-1 * inVal / multFactor + ORIGIN.getY() + thisImageView.getFitHeight()
				- thisImageView.getFitHeight() / multFactor);
	}

	private double lineOffsetY(double inVal, ImageView thisImageView) {
		return (-1 * inVal / multFactor + ORIGIN.getY() + thisImageView.getFitHeight() / 2
				+ thisImageView.getFitHeight() - thisImageView.getFitHeight() / multFactor);
	}

	// ==================================================================================
	private boolean outOfBounds(ImageView thisImageView) {
		return thisImageView.getX() < 0 || thisImageView.getX() > width || thisImageView.getY() < 0
				|| thisImageView.getY() > height;
	}
}
